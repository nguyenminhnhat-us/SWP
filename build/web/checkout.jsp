    ...
    <div class="container my-4">
        <div class="invoice-box">
            ...
            <div class="text-end mt-4">
                <div class="mb-3">
                    <h5 style="color: #333;">Chọn phương thức thanh toán:</h5>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="paymentMethod" id="vnpay" value="VNPAY" checked>
                        <label class="form-check-label" for="vnpay" style="color: #333;">
                            Thanh toán online qua VNPAY
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="paymentMethod" id="cod" value="COD">
                        <label class="form-check-label" for="cod" style="color: #333;">
                            Thanh toán khi nhận hàng (COD)
                        </label>
                    </div>
                </div>
                
                <form id="paymentForm" action="${pageContext.request.contextPath}/cart" method="post" class="d-inline">
                    <input type="hidden" name="action" value="complete-order">
                    <input type="hidden" name="amount" value="${total}">
                    <input type="hidden" name="payment_method" id="selectedPaymentMethod" value="VNPAY">
                    <button type="submit" class="btn btn-success" id="submitBtn">Xác nhận và Thanh toán VNPAY</button>
                </form>
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    function updatePaymentMethod() {
                        if (vnpayRadio.checked) {
                            submitBtn.textContent = 'Xác nhận và Thanh toán VNPAY';
                            paymentForm.action = '${pageContext.request.contextPath}/ajaxServlet';
                            selectedPaymentMethod.value = 'VNPAY';
                        } else if (codRadio.checked) {
                            submitBtn.textContent = 'Xác nhận đặt hàng (COD)';
                            paymentForm.action = '${pageContext.request.contextPath}/cart';
